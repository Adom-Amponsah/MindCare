  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-4 overflow-hidden bg-gradient-to-b from-blue-900 via-blue-700 to-blue-500">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute left-1/4 bottom-32 w-72 h-40 bg-pink-400 rounded-full filter blur-3xl opacity-30 animate-blob" style={{animationDelay: '1s'}}></div>
        <div className="absolute right-1/4 bottom-20 w-60 h-32 bg-yellow-300 rounded-full filter blur-2xl opacity-20 animate-blob" style={{animationDelay: '2s'}}></div>
        <div className="absolute left-1/2 top-24 w-80 h-32 bg-blue-300 rounded-full filter blur-2xl opacity-20 animate-blob" style={{animationDelay: '3s'}}></div>
      </div>

      {/* Floating Trust Indicators - Scattered across screen */}
      {trustIndicators.map((item, i) => (
        <div
          key={item.label}
          className={`absolute z-20 backdrop-blur-md bg-white/30 border border-white/40 shadow-xl rounded-full px-6 py-3 flex items-center gap-2 text-white font-semibold text-base drop-shadow-lg animate-float-${i + 1} ${
            i === 0 ? 'top-[40%] left-[15%]' : 
            i === 1 ? 'top-[85%] right-[10%]' : 
            'top-[60%] left-[8%]'
          }`}
      ...
      <div className="absolute top-[15%] right-[25%] z-20 backdrop-blur-md bg-white/20 border border-white/30 shadow-lg rounded-full px-4 py-2 flex items-center gap-2 text-white font-medium text-sm drop-shadow-lg animate-float-extra-1">
        <div className="w-3 h-3 bg-[#fbbf24] rounded-full"></div>
        <span>Trusted</span>
      </div>
      
      <div className="absolute top-[60%] right-[20%] z-20 backdrop-blur-md bg-white/20 border border-white/30 shadow-lg rounded-full px-4 py-2 flex items-center gap-2 text-white font-medium text-sm drop-shadow-lg animate-float-extra-2">
        <div className="w-3 h-3 bg-[#f472b6] rounded-full"></div>
        <span>Safe Space</span>
      </div>
      
      <div className="absolute top-[45%] left-[80%] z-20 backdrop-blur-md bg-white/20 border border-white/30 shadow-lg rounded-full px-4 py-2 flex items-center gap-2 text-white font-medium text-sm drop-shadow-lg animate-float-extra-3">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="text-center space-y-8 animate-fade-in-up">
          ...
          <div className="space-y-4">
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-poppins font-bold text-white leading-tight drop-shadow-xl">
              Your Mind Matters,{' '}
              <span className="text-[#fbbf24] inline-block breathe">Chale!</span>
            </h1>
            <p className="text-lg md:text-xl text-white/90 font-inter max-w-2xl mx-auto leading-relaxed">
        ...
        <div className="mt-20 animate-fade-in-up" style={{animationDelay: '0.9s'}}>
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 max-w-md mx-auto shadow-lg border border-[#fbbf24]/20">
            <div className="flex items-start space-x-4">
              <div className="w-12 h-12 bg-gradient-to-br from-[#fbbf24] to-[#f472b6] rounded-full flex items-center justify-center flex-shrink-0">
                <span className="text-white font-bold text-lg">A</span>
              </div>
              <div>
                <p className="text-[#2563eb] font-inter text-sm leading-relaxed mb-2">
                  "MindCare help me feel better, no judgment. The AI understand me when I talk Twi!"
                </p>
                <div className="text-xs text-[#2563eb]/80">
                  <span className="font-medium">Ama, 22</span> 
                </div>
              </div>
            </div>
          </div>
